...
const CONFIG = {
  ...
  customer: { first: 'Patrick', full: 'Patrick Sample' },
  ...
  tiers: [
    ...
    },
    {
      ...
      term: 'free to use',
      tagline: 'Alerts if your details show up where they should not',
      blurb: 'Monitoring across credit files and known breach lists. Unlocked and free to use.'
    },
    {
      goal: 10,
      name: 'Credit Optimizer Premium',
      term: 'free to use',
      tagline: 'Everything in Credit Optimizer, plus a coach',
      blurb: 'One-to-one sessions and a plan tailored to you. Unlocked and free to use.'
    }
  ],
...
const NOTIFICATIONS = {
  ...
  /* 1 — Momentum. An unfinished set is uncomfortable; lead with the count. */
  nudge1: {
    label: '1 · Momentum',
    icon: 'brand',
    title: function () {
      if (state.winsEarned === 0) return 'Welcome to easyWins, ' + CONFIG.customer.first;
      if (winsLeft() === 1) return 'One away, ' + CONFIG.customer.first;
      return 'You’re ' + state.winsEarned + ' for ' + state.winsEarned + ', ' + CONFIG.customer.first;
    },
    text: function () {
      if (state.winsEarned === 0) {
        return goal() + ' payments unlocks ' + currentTier().name + ', free.';
      }
      if (winsLeft() === 1) {
        return 'The next payment unlocks ' + currentTier().name + '.';
      }
      return winsLeft() + ' more and ' + currentTier().name + ' is yours.';
    }
  },

  /* 2 — The reward, named, with a date to act on. */
  nudge2: {
    label: '2 · Reward',
    icon: 'brand',
    title: function () {
      return winsLeft() === 1
        ? 'This one unlocks it'
        : currentTier().name + ' is ' + winsLeft() + ' away';
    },
    text: function () {
      return '$' + money(CONFIG.loan.payment) + ' due ' + fmtShort(state.dueDate) +
             (winsLeft() === 1 ? ' → ' + currentTier().name + '.' : '.');
    }
  },

  /* 3 — Payday. Urgency plus the smallest possible time cost. */
  nudge3: {
    label: '3 · Payday',
    icon: 'brand',
    primary: true,
    title: function () {
      if (winsLeft() === 1) return 'Payday. Last one.';
      if (state.winsEarned === 0) return 'Payday. One minute.';
      return 'Payday. Lock in easyWin ' + nextWin() + '.';
    },
    text: function () {
      if (winsLeft() === 1) {
        return 'Pay $' + money(CONFIG.loan.payment) + ' and ' + currentTier().name + ' is yours.';
      }
      if (state.winsEarned === 0) {
        return 'Pay $' + money(CONFIG.loan.payment) + ' → easyWin 1 of ' + goal() + '.';
      }
      return '$' + money(CONFIG.loan.payment) + ', takes a minute.';
    }
  },
  ...
  milestone: {
    ...
    title: function () {
      ...
      return t.name + ' is yours, ' + CONFIG.customer.first;
    },
  ...
  missed: {
    label: 'Back to zero',
    icon: 'brand',
    title: function () { return 'Back to zero, ' + CONFIG.customer.first; },
    text: function () {
      return goal() + ' easyWins unlocks ' + currentTier().name + '. Start today.';
    }
  }
};
